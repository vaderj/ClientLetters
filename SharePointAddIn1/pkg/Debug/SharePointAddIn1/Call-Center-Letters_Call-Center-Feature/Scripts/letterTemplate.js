...
var letterListName = "Letters"; /*= "CFS-CST Letters" ;
...
function buildLetterBody(letterObject)
{
	...
	for (var i = 0 ; i < letterTypes.length ; i++)
	{
		if (letterTypes[i].letterName == letterType)
		{
            ...
            console.log("unformatted letter")
            console.dir(unformattedLetter);
		}
	}
...
function getConfig()
{
	...
	lettersConfig.done(function(letterConfigData, textStatus, jqXHR){
		...
		{
			if(config[i].Key == "Token Mapping")
			{
				//console.log(config[i].Value)
				letterTokenMappings.push({
					token : config[i].Title,
					fieldInternalName : config[i].Value1
				}) ;
			}
			else if (config[i].Key == "Letter")
			{
				letterTypes.push({
			...
			else if (config[i].Key == "Font Size")
			{
				fontSize.push({
